 ...
 *
 * Sections carry ids and a sticky contents rail on wide screens: this page is
 * read out of order, by someone hunting one answer.
 */
import { PRICING, config } from "../config.js";
import { esc, page } from "./shell.js";

const SECTIONS: ReadonlyArray<[id: string, label: string]> = [
  ["paying", "How paying works"],
  ["endpoints", "Endpoints"],
  ["verdict", "The verdict"],
  ["clients", "Clients"],
  ["errors", "Errors"],
...
/** Code with a terminal frame and a copy button, matching the rest of the site. */
function term(name: string, code: string): string {
...
const table = (inner: string): string => `<div class="table-scroll"><table>${inner}</table></div>`;
...
export function renderDocs(): string {
  ...
  const toc = `<nav class="toc" aria-label="On this page"><ol>
    ${SECTIONS.map(([id, label]) => `<li><a href="#${id}">${esc(label)}</a></li>`).join("")}
  </ol></nav>`;

  const body = `
<div class="wrap">

<header style="padding-bottom:clamp(28px,4vw,40px)">
  <p class="eyebrow"><b>docs</b> three endpoints, one response shape</p>
  <h1 style="font-size:clamp(30px,4.5vw,46px)">Documentation</h1>
  <p class="lede">
    Paid per call. Nothing to sign up for — payment is the authentication.
  </p>
  <div class="facts">
    <span class="fact">Base <b>${esc(base)}</b></span>
    <span class="fact">Network <b>Algorand ${esc(net)}</b></span>
    <span class="fact">Asset <b>USDC &middot; ASA ${esc(usdcId)}</b></span>
  </div>
</header>
...
<section id="paying">
  <p class="eyebrow"><b>01</b> payment</p>
  ...
  ${term(
    "the 402 handshake",
    `<span class="dim"># 1. Ask without paying</span>
POST ${esc(base)}/v1/judge/transaction
<span class="dim">&rarr; 402 Payment Required</span>
<span class="dim">&rarr; PAYMENT-REQUIRED: &lt;base64 terms: amount, asset, payTo, network&gt;</span>

<span class="dim"># 2. Client signs a USDC payment and retries with proof</span>
POST ${esc(base)}/v1/judge/transaction
     X-PAYMENT: &lt;signed payload&gt;
<span class="dim">&rarr; 200 OK, verdict body</span>`,
  )}
  <p class="sub" style="margin:18px 0 22px">
  ...
  ${table(`
    ...
    <tr><td>Asset</td><td><code>USDC &middot; ASA ${esc(usdcId)}</code></td></tr>
...
<section id="endpoints">
  <p class="eyebrow"><b>02</b> surface</p>
  <h2>Endpoints</h2>
  ${table(`
  ...
  <h3 style="margin-top:30px">Transaction — Algorand</h3>
  ${term(
    "request",
    `{
  ...
  <h3 style="margin-top:26px">Transaction — EVM</h3>
  ${term(
    "request",
    `{
  ...
  <span class="k">"transaction"</span>: {
    <span class="k">"to"</span>: <span class="k">"0x&hellip;"</span>,
    <span class="k">"data"</span>: <span class="k">"0x&hellip;"</span>,                      <span class="dim">// omit for a plain value transfer</span>
    <span class="k">"value"</span>: <span class="k">"0"</span>
  }
}`,
  )}

  <h3 style="margin-top:26px">Counterparty</h3>
  ${term(
    "request",
    `{
  ...
  <h3 style="margin-top:26px">Human</h3>
  ${term(
    "request",
    `{
  <span class="k">"question"</span>: <span class="k">"Does this photo show a package at a front door?"</span>,
  <span class="k">"attachments"</span>: [<span class="k">"https://&hellip;"</span>],
...
<section id="verdict">
  <p class="eyebrow"><b>03</b> contract</p>
  <h2>The verdict</h2>
  ...
  <span class="k">"risk"</span>: 100,                <span class="dim">// 0&ndash;100</span>
  <span class="k">"confidence"</span>: 1,            <span class="dim">// 0&ndash;1</span>
  <span class="k">"findings"</span>: [{
    ...
    <span class="k">"title"</span>: <span class="k">"&hellip;"</span>,
    <span class="k">"detail"</span>: <span class="k">"&hellip;"</span>,
  ...
  ${table(`
    ...
    <tr><td><span class="badge allow">ALLOW</span></td><td>Nothing blocking was found. Proceed.</td></tr>
    <tr><td><span class="badge warn">WARN</span></td><td>Proceed only if your operator accepts the listed risks.</td></tr>
    <tr><td><span class="badge block">BLOCK</span></td><td>Do not proceed. Acting on this would likely cause loss.</td></tr>
    <tr><td><span class="badge escalate">ESCALATE</span></td><td>Not enough evidence to decide. <strong>Not an approval.</strong> Ask a human.</td></tr>`)}
  </div>
...
<section id="clients">
  <p class="eyebrow"><b>04</b> clients</p>
  <h2>Clients</h2>
  ${table(`
  ...
  <h3 style="margin-top:28px">Spend limits are part of the client</h3>
  <p class="sub">
  ...
  <h3 style="margin-top:28px">Python and other languages</h3>
  <p class="sub">
  ...
  ${term(
    "shell",
    `ARBITER_URL=${esc(base)} \\
ARBITER_PRIVATE_KEY=&hellip; \\
npx arbiter-proxy`,
  )}
</section>

<section id="errors">
  <p class="eyebrow"><b>05</b> failure modes</p>
  <h2>Errors</h2>
  ${table(`
...
<section id="reviewer">
  <p class="eyebrow"><b>06</b> supply side</p>
  <h2>Reviewer API</h2>
  <p class="sub">
